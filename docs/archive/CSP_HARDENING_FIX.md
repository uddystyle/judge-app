...
**問題**: 初期のNonce-based CSP実装に重大なバグがあり、ページ遷移後のクリックナビゲーションが動作しなくなりました。

**原因**: SvelteKitが生成するクライアントサイドナビゲーション用スクリプトにnonceが適用されず、すべてブロックされました。

**修正**: `unsafe-inline` を再追加し、`unsafe-eval` のみ削除する方針に変更しました。

**詳細**: `CSP_NAVIGATION_FIX.md` を参照してください。
...
const cspDirectives = [
    "default-src 'self'",
    "script-src 'self' 'unsafe-inline' https://js.stripe.com",  // unsafe-inline 復活
    "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
    "font-src 'self' https://fonts.gstatic.com",
    "img-src 'self' data: https: blob:",
    "connect-src 'self' https://*.supabase.co https://api.stripe.com wss://*.supabase.co",
...
- ⚠️ `unsafe-inline` 許可（SvelteKit要件）
- ✅ 外部リソースの厳格なホワイトリスト化

---

**次のステップ**:
- SvelteKit公式CSP実装（`svelte.config.js` の `kit.csp`）への移行調査
- style-src の Nonce対応
- CSP Violation監視の実装

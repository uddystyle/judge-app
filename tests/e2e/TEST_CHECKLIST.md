...
- [ ] 現在の`plan_type`: ******\_\_\_******
- [ ] 現在の`stripe_subscription_id`: ******\_\_\_******
...
### PWA / Service Worker（2026-08-02 追加）

- [ ] 本番ドメインで DevTools → Application → Service Workers に tento の SW が activated で表示される
- [ ] 2回目以降のアクセスで /\_app/ 資産が SW キャッシュから返る（Network タブで "ServiceWorker" 表示）
- [ ] オフラインで未訪問ページへ遷移 → ブラウザエラーではなく「オフラインです」ページが表示され、未同期件数と案内が出る
- [ ] オフラインページの「再読み込み」で復帰後に元のページが表示される
- [ ] 採点 POST・/api/\*・Supabase 通信が SW にキャッシュされない（オフライン採点の挙動がインクリメント1-3 と変わらない）
- [ ] ホーム画面追加時に TENTO アイコン（青地に T）と名前が表示され、standalone で起動する（iOS/Android）
- [ ] デプロイ後、全タブを閉じて開き直すと新しいバージョンの SW に更新される

...
> rollback は通常運用では実行しない（緊急時のみ）。forward とペアで保管する。複数適用済みの場合は**番号の大きい方から**戻す（各 rollback ヘッダの指示に従う）。
...
> これらは前進スキーマ変更ではない。整理プラン Phase 1 で `verify/`・`database/diagnostics/`・`archive/one-time/` への移設候補。

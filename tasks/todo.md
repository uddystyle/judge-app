...
### 本番との突合結果（2026-08-04・ユーザー確認済み）

- [x] **本番(live mode)の年額も新価格（¥88,000 / ¥248,000 / ¥498,000）だった**
  - つまり**請求は最初から正しく、画面表示（旧: ¥70,000 / ¥180,000 / ¥300,000）だけが誤っていた**
  - Stripe 側の Price 新規作成・Vercel 環境変数の差し替えは**不要**。今回の表示修正で乖離は解消
  - test と live で価格が揃っていることも確認できた（ステージング検証が本番を反映する）
...
- [x] legal / terms: デプロイ承認により公開（文言はいつでも修正可）
- [x] デプロイ: 2026-08-01 main へ push（2d78543）→ Vercel 自動デプロイ。DB 先行の順序どおり
...
- [x] 1.【中】アップグレード時に旧Stripeサブスクリプションが解約されず二重課金
  ...
  - SEC-1b（レビュー指摘の追補）: 新サブスクが incomplete（非同期決済未確定）の場合は解約をスキップ
    （非同期決済の失敗で組織が課金中サブスクを失うのを防ぐ）。スキップ時は console.error で高重大度記録。
    非同期決済が後で成功した場合の旧サブスク自動解約は未実装＝二重課金がその経路にのみ残る
    （JPYサブスクは実質カードのみで発生確率が極めて低く、payment_succeeded/subscription.updated の
    到着順レースを跨ぐ実装コストに見合わないため、ログの運用検知＋手動解約で対応）
...
## #7 恒久: results owner 列（2段階・無停止）バッチ（2026-06-27）
...
- [x] アプリ（owner を書く/読む）:
  - `input/+page.server.ts` 大会分岐：upsert+retry → owner で SELECT→UPDATE/INSERT＋23505フォールバック。owner 行が無いのに 23505＝phase1 の旧 name-unique 同名衝突は 409。
  - 旧フロー `[discipline]/[level]/[event]/score/+page.server.ts`：同方式へ（生エラー露出も解消）。
  - `results/+page.server.ts` 大会 myScores：judge_name → owner（judge_id/guest_identifier）。
- [x] `1010_results_owner_enforce.sql`（phase2）＋ `1010_rollback.sql`
  - 旧 name-unique（results_unique_score_entry/results_unique_score/unique_result_per_judge）撤去。RLS の INSERT/UPDATE を owner 基準へ（anon=guest_identifier=JWT、authed=judge_id=auth.uid()＋session参加）。SELECT/DELETE は既存維持。
...
## #8: sessions FK 整備 バッチ（2026-06-27）
...
- [x] **base sessions schema 収録**：prod の実DDL（列24・制約・索引）を正本に `0000_base_sessions_schema.sql` 作成（CREATE系のみ・冪等・no-op）。
...
## join レート制限の会場共有IP対策 バッチ（2026-06-27）
...
## 多審制 finalize 人数ゲート（M1）＋ required_judges を検定で機能させる（M2）バッチ（2026-06-28）
...
- [x] `api/score-status`：`required_judges || 1` で null 安全化。
...
## 検定(legacy)に active-prompt 認可ゲート移植（M3）＋ setPrompt エラー隠蔽（L1）バッチ（2026-06-28）
...
## 旧検定(certification) requestCorrection 削除を owner 基準へバッチ（2026-06-29）

...
- [ ] **base sessions schema 収録**：突合の列一覧が不完全の疑い（join が使う failed_join_attempts/is_locked が無い）。実DDLをユーザーから受領 → `0000_base_sessions_schema.sql` 作成（保留）。

### 運用（ユーザー実行・未実施）
- [ ] **DEV に 1011 適用** → cleanup notice 確認＋FK が auth.users SET NULL になったこと。テストユーザー削除で chief/created_by が NULL・セッション/results 残存・appointChief 再設定可。
- [ ] **prod に 1011 適用**（順序制約なし）。問題時 1011_rollback。
- [ ] base schema 用に prod の `pg_dump --schema-only -t public.sessions`（または完全な列/制約/索引ダンプ）を貼る → 収録。

<script lang="ts">
	...
	const CLEAR = 'clear';
	const SCORES = [88, 92, 76, 85];
	const FRAME_MS = 620;

	type Frame = { display: string; active: string | null; confirmed: boolean };

	// 各得点について「1桁ずつ入力 → 確定 → 保持 → リセット」のフレーム列を作る
	function buildFrames(scores: number[]): Frame[] {
		const frames: Frame[] = [];
		for (const score of scores) {
			frames.push({ display: '', active: null, confirmed: false });
			let display = '';
			for (const digit of String(score)) {
				display += digit;
				frames.push({ display, active: digit, confirmed: false }); // キー押下
				frames.push({ display, active: null, confirmed: false }); // 離す
			}
			frames.push({ display, active: CONFIRM, confirmed: false }); // 確定押下
			frames.push({ display, active: null, confirmed: true }); // 確定済み（弾む＋チェック）
			frames.push({ display, active: null, confirmed: true }); // 保持
		}
		return frames;
	}

	const frames = buildFrames(SCORES);
	// 静止時（reduced-motion / 初期）は最初の得点が入った状態を見せる
	const staticFrame: Frame = { display: String(SCORES[0]), active: null, confirmed: true };

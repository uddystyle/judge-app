...
describe('非同期決済ガード（SEC-1b: incomplete時は旧サブスクを解約しない）', () => {
	...
	it('新サブスクリプションがincomplete（非同期決済未確定）の場合は旧サブスクを解約しない', async () => {
		vi.mocked(stripe.webhooks.constructEvent).mockReturnValue({
			id: 'evt_upgrade_async',
			type: 'checkout.session.completed',
			livemode: false,
			data: {
				object: {
		...
		vi.mocked(stripe.subscriptions.list).mockResolvedValue({
			data: [
				{ id: 'sub_old_1', status: 'active' },
				{ id: 'sub_new_1', status: 'incomplete' }
			]
		} as any);
